		...
		for (Object agent : allObjects) {
			if (agent.getClass() == DroneAgent.class) {
				...
				for (CollisionsSensor sensor : sensors) {
					...
					graphics.setPaint(distance < 0 ? new Color(0.5f, 0.5f, 0.5f, 0.5f) : new Color(1f, 1f, 1f, 0.5f));

					graphics.drawLine((int) (info.draw.width * position.x / Constants.MAP_WIDTH),
							(int) (info.draw.height * position.y / Constants.MAP_HEIGHT),
							(int) (info.draw.width * (position.x + distance * Math.cos(sensorAbsoluteAngle))
									/ Constants.MAP_WIDTH),
							(int) (info.draw.height * (position.y + distance * Math.sin(sensorAbsoluteAngle))
